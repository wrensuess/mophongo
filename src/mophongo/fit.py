...
class FitConfig:
    ...
    solve_method: str = "scene"  # only 'scene' is implemented (see docs/dead_code.md)
...
class SparseFitter:
    ...
    def solve(self, config: FitConfig | None = None) -> Tuple[np.ndarray, np.ndarray, dict]:
        ...
        raise ValueError(
            f"Unsupported solve_method {config.solve_method!r}; only 'scene' is "
            "implemented (legacy 'ata'/'all'/'lo' paths were removed — see "
            "docs/dead_code.md)."
        )
